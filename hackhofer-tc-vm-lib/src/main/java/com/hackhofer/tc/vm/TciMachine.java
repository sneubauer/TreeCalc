...
import com.hackhofer.tc.rt.B;
import com.hackhofer.tc.rt.ExceptionNeedMoreInput;
import com.hackhofer.tc.rt.S;
import com.hackhofer.tc.rt.V;
import com.hackhofer.tc.rt.VDouble;
import com.hackhofer.tc.rt.VList;
import com.hackhofer.tc.rt.VNull;
import com.hackhofer.tc.vm.asm.TciAsmReaderWriter;
import com.hackhofer.tc.vm.asm.TciAssembler;
import com.hackhofer.tc.vm.asm.TciAssembler.Asm;
import com.hackhofer.tc.vm.asm.TciAssembler.Calc;
import com.hackhofer.tc.vm.asm.TciAssembler.Func;
import com.hackhofer.tc.vm.asm.TciAssembler.Input;
import com.hackhofer.tc.vm.asm.TciAssembler.Instruction;
import com.hackhofer.tc.vm.asm.TciAssembler.Node;
import com.hackhofer.tc.vm.asm.TciAssembler.NodeCalc;
import com.hackhofer.tc.vm.asm.TciAssembler.Table;
...
//	public static final boolean TRACE=false;
	...
	public String calculateMaintree(String name) {
		...
		if(calcindex>=0) {
			...
			throw new RuntimeException("invalid calculation name" + name);
		}
	}
	...
	private static char getInteger(CharacterIterator i, char c, StringBuffer token) {
		if (!Character.isDigit(c)) {
			throw new RuntimeException("invalid argument: Integer expected");
		}
	...
	private static char getExponent(CharacterIterator i, char c, StringBuffer token) {
		// EXPONENT => e[-]?[0-9]+
		if (c!='e' && c!='E')
			throw new RuntimeException("invalid argument: exponent expected");
		token.append('e');
		c=i.next();
		if (c=='-') {
			token.append('-');
	...
	public String calculateInput(final String name) {
		...
		final int inputid = getInputIndex(justnameUpper);
		if (inputid<0) {
			throw new RuntimeException("Invalid inputname " + name);
		}
		V[] vindex;
		if (index==null) {
			vindex = new V[0];
	...
	public String calculateInputCalc(String name, String calcname) {
		...
		if (inputcalcid<0) {
			throw new RuntimeException("invalid calcname " + calcname);
		}
...
//		V[] args = (V[]) parseRet[1];
		...
		throw new RuntimeException("invalid calculation string '" + name + "'");
	}
	@Override
	...
	public String calculate(long id, String... args) {
		...
		throw new RuntimeException("invalid calculation id: " + id);
	}
	@Override
	...
	public boolean needed(String inputname, String calcname) {
		...
		final int inputid = getInputIndex(justnameUpper);
		if (inputid<0) {
			throw new RuntimeException("Invalid inputname " + inputname);
		}
		int indexlen = index.length;
		V[] vindex = new V[indexlen];
		for (int i=0; i<indexlen; i++) {
...
//		int start = asm.formulaOffset[formula];
		if (vargs.length!=nargs) {
			throw new RuntimeException("Function " + func.name + " expects " + nargs + " arguments, but got " + vargs.length + ", please check the interface call.");
		}
	...
	private int getFuncIndex(String funcnameUpper) {
		...
		if (i==null) {
			throw new RuntimeException("invalid function name: " + funcnameUpper);
		}
		return i;
	}
...
//		ksp += 1;
		...
		for (;;) { //interpreter loop
			...
			case INSTR_BUILTIN                     : //"builtinid", OPTYPE_BYTE, "nargs", OPTYPE_INT) //arg1 ... arg_nargs -- result
				...
				case 3: // "bf_select"
					throw new RuntimeException(op1 + " not implemented yet in TciMaschine");
				case 4: // "bf_selectx"
					throw new RuntimeException(op1 + " not implemented yet in TciMaschine");
				case 5: // "bf_v_select"
					throw new RuntimeException(op1 + " not implemented yet in TciMaschine");
